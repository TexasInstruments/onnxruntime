...
### Prerequisite
```bash
pip install --no-input --upgrade pip setuptools cmake numpy packaging pybind11 wheel numpy
```
...
$ cd ../
$ tar -czf onnx_1.15.0_x86_u22.tar.gz onnx_1.15.0_x86_u22/
```
...
SET(CMAKE_C_COMPILER  <PATH_TO_SDK>/toolchain/sysroots/x86_64-arago-linux/usr/bin/aarch64-oe-linux/aarch64-oe-linux-gcc) # SDK toolchain
SET(CMAKE_CXX_COMPILER  <PATH_TO_SDK>/toolchain/sysroots/x86_64-arago-linux/usr/bin/aarch64-oe-linux/aarch64-oe-linux-g++) # SDK toolchain
SET(CMAKE_SYSROOT <PATH_TO_SDK>/targetfs) # SDK target filesystem
include_directories (<PATH_TO_SDK>/targetfs/usr/lib/python3.12/site-packages/numpy/core/include) # Numpy dependency from SDK
```

> [NOTE]
> - <PATH_TO_SDK> is full path to sdk base directory. Ex: /home/user/ti-processor-sdk-rtos-j721s2-evm-11_01_01_01/
> - The python version might change between SDKs, hence make sure to update the python path for numpy include directories
...
```bash
$ mkdir -p ~/onnx_1.15.0_aragoj7
$ cd ~/onnx_1.15.0_aragoj7
...
$ cd ../
$ tar -czf onnx_1.15.0_aragoj7.tar.gz onnx_1.15.0_aragoj7/
```
